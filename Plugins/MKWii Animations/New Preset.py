...
def CreateScripts(path, name, mat, textureText):
    with StreamWriter(Path.Combine(path, "Import.py")) as sr:
        ...
        "                wrapper.Resource.Replace(Path.Combine(BrawlAPI.PluginPath, \"MKWii Animations\", \"" + name + "\", \"" + textureNames[len(textureNames)-1] + ".tex0\"))\n"
        "                wrapper.Resource.Name = \"" + textureNames[len(textureNames)-1] + "\"\n"
        "                return\n\n"
        "def ImportTextures():\n"
        "    wrappers = BrawlAPI.NodeWrapperList\n"
        "    for wrapper in wrappers:\n"
