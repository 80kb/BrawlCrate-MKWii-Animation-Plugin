    ...
    for wrapper in wrappers:
        if ".mdl0" in wrapper.ExportFilter:
            if wrapper.Resource.Name == "course":
                ...
                shader.Replace(Path.Combine(BrawlAPI.PluginPath, "MKWii Animations", "MKWii Half Pipe", "mkwBlueShader.mdl0shade"))
                return shader.Name
    ...
    for wrapper in wrappers:
        if ".png" in wrapper.ExportFilter:
            if wrapper.Resource.Name == children[0].Name:
                wrapper.Resource.Replace(Path.Combine(BrawlAPI.PluginPath, "MKWii Animations", "mkwBlueBumpS.tex0"))
                wrapper.Resource.Name = "mkwBlueBumpS"
                return
...
def ImportTextures():
    ...
    for wrapper in wrappers:
        if ".brres" in wrapper.ExportFilter:
            g_node = BrawlLib.SSBB.ResourceNodes.NodeFactory.FromFile(None, Path.Combine(BrawlAPI.PluginPath, "MKWii Animations", "mkwBlueGradS.tex0"))
            wrapper.Resource.GetOrCreateFolder[BrawlLib.SSBB.ResourceNodes.TEX0Node]().AddChild(g_node)
            r_node = BrawlLib.SSBB.ResourceNodes.NodeFactory.FromFile(None, Path.Combine(BrawlAPI.PluginPath, "MKWii Animations", "mkwBlueRainbow.tex0"))
            wrapper.Resource.GetOrCreateFolder[BrawlLib.SSBB.ResourceNodes.TEX0Node]().AddChild(r_node)
            c_node = BrawlLib.SSBB.ResourceNodes.NodeFactory.FromFile(None, Path.Combine(BrawlAPI.PluginPath, "MKWii Animations", "mkwBlueBoost.srt0"))
    ...
    for wrapper in wrappers:
        if ".mdl0mat" in wrapper.ExportFilter:
            ...
            if wrapper.Resource.Name == x:
                ...
                ImportTextures()
                wrapper.Resource.Replace(Path.Combine(BrawlAPI.PluginPath, "MKWii Animations", "mkwBlueBoost.mdl0mat"))
                for i in range(MatAmount() - CurrentIndex):
                    wrapper.Resource.DoMoveDown()
                wrapper.Resource.Shader = CreateShader()
                wrapper.Resource.Name = "mkwBlueBoost"
                return
